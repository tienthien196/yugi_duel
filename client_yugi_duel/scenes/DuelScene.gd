...
onready var lbl_phase = $CenterBoard/PhaseIndicator/LabelCurrent
...
func _on_state(state):
	...
	# cập nhật hand (giả sử state.players[my_id].hand = array card_id)
...
func _on_play_card(cid):
	GameClientController.play_monster(cid, 0) # demo: summon vào zone 0
